...
export function TransactionRow({ 
    ...
    const gridCols = showAsset 
        ...
        : "grid-cols-[1.2fr_0.8fr_1fr_40px] md:grid-cols-[1fr_1fr_1fr_1.2fr_1.2fr_0.8fr_80px]";
    ...
    return (
        ...
        >
            <div className="flex flex-col md:block">
                {showAsset && <div className="font-bold text-sm md:text-base tracking-tight truncate">{tx.symbol}</div>}
            ...
            <div className="flex justify-end gap-0.5 md:opacity-0 md:group-hover:opacity-100 transition-all">
                ...
                <Dialog open={isEditing} onOpenChange={setIsEditing}>
                    <DialogTrigger asChild>
                        <Button variant="ghost" size="icon" className="h-7 w-7 md:h-8 md:w-8 hover:bg-background/80" onClick={(e) => { e.stopPropagation(); onEdit(tx.id); }}>
                            <Edit className="h-3.5 w-3.5 md:h-4 md:w-4 text-muted-foreground" />
                        </Button>
                    </DialogTrigger>
                ...
                <Button variant="ghost" size="icon" className="h-7 w-7 md:h-8 md:w-8 hover:text-destructive hover:bg-destructive/5" onClick={(e) => onDelete(tx.id, e)}>
